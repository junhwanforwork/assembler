...
**실수노트**(REPORT 수집): `git rm --cached`를 의도 없이 실행할 뻔 — 직후 복구, 커밋 오염 0. 파일 재작성 전 습관적 명령 조합 주의.
...
- dangling 테스트 픽스처를 서버 실산출이 아닌 bare id로 "구현에 맞춰" 만들어 green을 자작 — 크로스 경계(서버 산출물)를 소비하는 코드는 **픽스처를 생산자 코드에서 역추적**해 만들 것. 테스트 파일에 경고 주석으로 고정.
