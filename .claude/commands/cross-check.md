---
description: 티켓 구현 완료 후 code-reviewer + howcloud-qa가 독립적으로 병렬 크로스체크한다.
---
...
- **API 파일 포함 여부**: `src/app/api/` 하위 파일 존재 시 → `howcloud-be` 추가
...
```
howcloud 프로젝트 크로스체크 — 코드 품질·보안 리뷰
...
다음 프롬프트로 `howcloud-qa` 에이전트를 호출한다:

```
howcloud 프로젝트 크로스체크 — QA 커버리지 리뷰
...
### 에이전트 C: howcloud-be (API 파일이 있을 때만)

API 또는 마이그레이션 파일이 포함된 경우에만 다음 프롬프트로 `howcloud-be` 에이전트를 호출한다:

```
howcloud 프로젝트 크로스체크 — 백엔드 / DB 리뷰
...
### 🟢 통과 항목
- code-reviewer: 보안 ✅ / 타입 ✅ / howcloud 규칙 ✅
- howcloud-qa: 해피패스 ✅ / 엣지케이스 ✅ / 정책 ✅
- howcloud-be: RLS ✅ / 쿼리 ✅ / 문서 ✅  (해당 시)
