---
description: 티켓 구현 완료 후 code-reviewer + assembler-qa가 독립적으로 병렬 크로스체크한다.
---
...
- **API 파일 포함 여부**: `src/app/api/` 하위 파일 존재 시 → `assembler-be` 추가
...
```
assembler 프로젝트 크로스체크 — 코드 품질·보안 리뷰
...
5. 로직 정확성
   - 콘텐츠 상태 머신 위반 (`is_published=false` 검수 우회 등)
   - FEATURE.md 결제·소비 정책 이탈
...
다음 프롬프트로 `assembler-qa` 에이전트를 호출한다:

```
assembler 프로젝트 크로스체크 — QA 커버리지 리뷰
...
5. CLAUDE.md 정책 준수
   - 콘텐츠 검수 흐름 (is_published)
   - 비로그인 저장(localStorage) → 로그인 마이그레이션 흐름
   - 공유 링크 스냅샷 읽기전용
...
### 에이전트 C: assembler-be (API 파일이 있을 때만)

API 또는 마이그레이션 파일이 포함된 경우에만 다음 프롬프트로 `assembler-be` 에이전트를 호출한다:

```
assembler 프로젝트 크로스체크 — 백엔드 / DB 리뷰
...
3. 트랜잭션 안전성
   - 콘텐츠 publish + 메타데이터 업데이트 같은 복합 write의 rollback 처리
   - 부분 실패 시나리오 처리
...
### 🟢 통과 항목
- code-reviewer: 보안 ✅ / 타입 ✅ / assembler 규칙 ✅
- assembler-qa: 해피패스 ✅ / 엣지케이스 ✅ / 정책 ✅
- assembler-be: RLS ✅ / 쿼리 ✅ / 문서 ✅  (해당 시)
