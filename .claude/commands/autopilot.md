...
4. **재시도 상한:** 빌드·기능 QA(Playwright)·코드리뷰 각 누적 2회 초과 시 STOP + 알림.
...
`memory/tickets.md`(`/Users/junhwanlim/.claude/projects/-Users-junhwanlim-Projects-assembler/memory/tickets.md`)를 읽는다.

- `$ARGUMENTS`에 티켓 ID가 있으면 그 티켓. 단 고위험 마커가 있으면 STOP + 알림.
- 없으면 선택 규칙(`automation-cycle.md`):
  1. `[~]` In Progress 티켓이 있으면 그것(재개 우선).
  2. 없으면 첫 `[ ]` todo 중 **모두 만족**: 고위험 마커 없음(`⚠`·`⏸` 보류 제외) · 선행 의존 `[x]` 완료 · 코드 변경으로 명확.
  3. 후보 없으면 → STOP, `PushNotification`("autopilot: 자동 가능한 티켓 없음").
- **Epic 헤딩(`### 🎯 EPIC ASS-EXX`)은 선택 대상 아님** — 체크박스 없는 그룹 레이블. 그 아래 `[ ]` 하위 `ASS-NNN`만 선택.
...
티켓이 새 기능·UX 변경이면 multi-team 진입 전 `assembler-design` UX 검증을 먼저 한다.
...
- 통과 → Step 6.
- 실패 → 정확한 에러 출력을 구현 에이전트(assembler-fe/be)에 전달 → 수정 → 재시도. **누적 2회** 초과 시 STOP + 알림(마지막 에러 요약 첨부).

## Step 6 — QA (기능 QA + 코드리뷰)

### 6a. 기능 QA — Playwright (HARD GATE, ASS-E18)

실제 브라우저로 기능이 동작하는지 검증한다. **정적 분석(코드리뷰)만으론 "동작 없음" 버그를 못 잡는다** — 실제 클릭·렌더 확인.

1. 이 티켓이 바꾼 기능에 대한 e2e spec을 **`e2e/<feature>.spec.ts`로 추가/갱신**한다.
   - **AI 호출 0 원칙:** `/preview` 픽스처·시드(`e2e/helpers.ts`의 `seedSession`)로 빌더를 띄워 검증. 생성 배선은 `mockGenerate`(`page.route`)로 — 실 `/api/generate`(opus) 호출 금지.
2. `npm run e2e` 실행 (스모크 + 티켓 spec). worktree에서 돌리면 `E2E_PORT`로 포트 override(dev 3017/3018과 분리).
3. **통과 → 6b.** 실패 → 실패 로그를 구현 에이전트에 전달 → 수정 → Step 5(빌드 게이트)부터 재시도. **누적 2회** 초과 시 STOP + 알림.

> 순수 비코드 티켓(문서·토큰만)이라 UI 동작 변화가 없으면 스모크만 돌리고 티켓 spec은 생략 가능(PR에 사유 명시).

### 6b. 코드리뷰 — /cross-check

`/cross-check`를 실행한다 (code-reviewer + assembler-qa 독립 병렬, API 변경 시 assembler-be 추가).
...
4. `/pr` 실행 — `gh pr create`로 PR 생성. 본문에 변경 요약 + **기능 QA(Playwright) 통과·추가한 e2e spec** + cross-check 판정(CONDITIONAL이면 HIGH 항목) + 테스트 방법.
...
QA·리뷰 병렬 구간을 더 결정론적으로 돌리고 싶으면 Step 6을 `/cross-check` 대신 **Workflow 툴**로 팬아웃한다 (code-reviewer·assembler-qa·assembler-be를 `parallel`로 스폰 → 판정 취합). 기본은 `/cross-check` 재사용.
