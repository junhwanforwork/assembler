...
1. **cwd 자동 감지:** 현재 폴더가 `.claude/worktrees/lane-N`(고정 레인 슬롯) 안이면 = **레인 세션** — 질문 없이 해당 레인으로 확정. 패킷 제약(로컬 커밋만, push·merge·tickets.md·DB 적용 금지) 리마인드 후, **워크트리 루트 `PACKET.md`가 있으면 그걸 착수 명세의 정본으로 로드**(`/lane-start`와 동일 규약 — 완료 보고도 `REPORT.md`로). PACKET.md가 없으면 "다음 패킷 대기 중" 상태로 안내하고 tickets.md 맥락만 요약.
2. **메인 폴더면** `git worktree list`로 레인 슬롯을 파악해 AskUserQuestion으로 역할을 묻는다:
   - `오케스트레이터` (Recommended) — 대기열 점검(/wave-status)·머지(/wave-integrate)·발주(/wave-prep)·티켓/문서. 선택 시 Step 1~3 정상 진행.
   - `레인 N` — 현존 lane-N 슬롯마다 1옵션(최대 3). 슬롯이 없으면 레인 옵션 생략.
