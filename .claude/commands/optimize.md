---
description: 진단·최적화 계획 — howcloud-optimizer로 현재 코드·구조·API·DB를 분석해 근본원인·공수·우선순위 리포트를 만든다. 코드는 안 고친다. 설계: docs/specs/optimization-planner.md.
---
...
## Step 1 — howcloud-optimizer 호출

`howcloud-optimizer` 에이전트를 호출한다. 전달:
- 이슈 서술 / 티켓 맥락
...
- **코드 수정 금지.** 이 커맨드와 howcloud-optimizer는 진단만 한다. 고치고 싶으면 인계 워크플로우로.
