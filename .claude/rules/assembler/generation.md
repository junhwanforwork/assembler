...
- **완전 페이지 분해 (ASS-106): navigate 대상 페이지는 반드시 생성**(고립 이동 금지). 제품을 메인 한 장이 아니라 *전체 페이지 집합*으로 분해 — 링크되는 모든 화면이 Page+Wireframe으로 존재해야 한다. normalize는 미존재 대상 navigate를 stub 페이지 자동생성 없이 요소 마킹으로 안전망 처리(`pruneDanglingRefs` 마킹 + `reconcileNavigateEdges` 정합).
