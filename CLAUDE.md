...
- **멀티터미널 병렬 = 오케스트레이터 경유.** 작업 세션은 오케스트레이터가 발급한 워크트리에서만 착수(패킷+/goal 동봉). 로컬 커밋은 자유, 머지·push·DB 적용·tickets.md는 오케스트레이터 독점. 착수 전 순서: main 머지→워크트리→패킷. 상세 = 사용자 메모리 multi-terminal-orchestration. **루틴은 `/wave-prep`(준비·패킷 발급)·`/wave-integrate`(머지·게이트·마감)로 실행.**
