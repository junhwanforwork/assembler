...
export function useEditorChat(workspaceId: string, onPlan: (plan: ChangePlan) => void) {
  ...
  // 마지막 실패 턴 재전송 — 사용자 메시지는 이미 목록에 있으므로 히스토리에서 그 턴만 뺀다.
  const retry = useCallback(() => {
    if (failedText === null || sendState === "sending") return
    void post(messages.slice(0, -1), failedText)
  }, [failedText, messages, post, sendState])
