...
export const DashboardShell: FC<DashboardShellProps> = (props) => {
  ...
  // 첫 방문(로드 완료·프로젝트 0개·검색 아님) = 프롬프트-퍼스트 히어로로 맞이한다(빈 그리드 대신).
  // 로그인이 들어오면 "익명 && 0개"로 좁힌다(현재는 전부 익명 세션).
  const isFirstVisit = state === "ready" && items.length === 0 && !hasAnyProjects;
    ...
    <div style={ROOT_STYLE}>
      ...
      <div style={BODY_STYLE}>
        ...
        <main style={MAIN_STYLE}>
          {isFirstVisit ? (
            <HeroEntry onGenerate={props.onGenerate} busy={props.creating} />
          ) : (
            <>
