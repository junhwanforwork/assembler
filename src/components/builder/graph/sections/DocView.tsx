...
// 선택은 로컬 state(문서 안에서만). 기본 = 첫 요구사항.
  ...
  return (
    <div style={GRID}>
      <aside style={COL_LIST} aria-label="요구사항 목록">
        ...
        {requirements.length === 0 ? (
          ...
          requirements.map((r) => {
            ...
            return (
              <button key={r.id} type="button" onClick={() => setSelectedId(r.id)} style={reqRow(active)}>
      ...
      <section style={COL_DETAIL} aria-label="요구사항 상세">
        {selected ? <RequirementDetail requirement={selected} graph={graph} /> : <p style={EMPTY}>요구사항을 선택해 주세요.</p>}
      </section>
    </div>
  )
}
...
const COL_LIST: CSSProperties = {
  width: 240, flexShrink: 0, height: "100%", overflowY: "auto", padding: SPACING["3"],
  borderRight: `1px solid ${COLOR.BORDER_DEFAULT}`, display: "flex", flexDirection: "column", gap: SPACING["1"],
}
const COL_TREE: CSSProperties = {
  width: 280, flexShrink: 0, height: "100%", overflowY: "auto", padding: SPACING["3"],
...
const COL_TITLE: CSSProperties = {
  ...TYPOGRAPHY.STYLE.LABEL_2, color: COLOR.TEXT_MUTED, textTransform: "uppercase", letterSpacing: "0.06em",
  margin: `${SPACING["1"]} 0 ${SPACING["2"]}`, paddingLeft: SPACING["2"],
}
...
function reqRow(active: boolean): CSSProperties {
  return {
    display: "flex", alignItems: "center", justifyContent: "space-between", gap: SPACING["2"], width: "100%",
    padding: `${SPACING["2"]} ${SPACING["2"]}`, border: "none", borderRadius: RADIUS.SM, cursor: "pointer", textAlign: "left",
    backgroundColor: active ? COLOR.ACCENT_BG : "transparent", color: active ? COLOR.ACCENT : COLOR.TEXT_SECONDARY,
    transition: INTERACTION.TRANSITION_BG,
  }
}
const REQ_LABEL: CSSProperties = { ...TYPOGRAPHY.STYLE.LABEL_1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }
const BADGE: CSSProperties = { ...TYPOGRAPHY.STYLE.LABEL_2, flexShrink: 0, minWidth: 18, textAlign: "center", color: COLOR.TEXT_MUTED }
