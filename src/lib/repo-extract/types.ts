...
// ASM-070 — 레포 기획 md 문서(경로 + 내용). 코드가 아니라 "문서만"이 안전 경계.
export type MarkdownDoc = { path: string; content: string }
...
// docs = 읽어온 기획 md 문서(ASM-070). payload가 아니라 report에 담는다 — payload는 싱크-인
// 파서(parseApiSync/parseDbTableSync)를 통과해야 하는 계약이라 문서를 넣으면 계약이 깨진다.
// optional이라 동결 시그니처로 만든 소비자 코드·스텁과 호환된다.
