...
// (다른 컴퓨터의 988f267 "ProjectGraph IDE 슬라이스"에서 이식 — 순수 함수라 로컬 타입에 그대로 맞물린다.
//  remote 원본은 store/graph의 selection 타입에 의존했으나, 로컬 store는 page/element 중심이라
//  이 모듈을 self-contained로 두고 selection 타입을 여기서 정의한다. UI 배선은 후속 티켓.)
//
// 방향:
...
/** 영향분석이 다루는 6종 객체 타입. (로컬 store는 page/element만 선택하므로 여기서 정의.) */
