...
| 선택 노드 | 뷰탭 | 기본 탭 |
|---|---|---|
| Page / Feature / 프로젝트 루트 | **[화면] [문서] [흐름]** | 노드 따라(Page→화면, Feature/요구→문서, 루트→흐름) |
| UI 요소 | [화면](부모 페이지+요소 포커스) | 화면 |
| API · Database | **[표]** (전용) | 표 |
...
- 뷰탭 집합: Page는 [화면][문서][흐름] 3종 확정. Feature/요구는 [문서] 단일? 루트는 [흐름] 단일? — 노드별 탭 셋 확정 필요.
