...
## 2. JSON 강제 = 프롬프트 유도 + 정규화 (structured outputs는 ProjectGraph에 비가용)

- ⚠ **라이브 확인(2026-06-14):** `output_config.format` 에 ProjectGraph 전체 스키마를 실으면 **`invalid_request_error: "The compiled grammar is too large"`(400)** — 12개 컬렉션 평면 스키마가 structured outputs 문법 컴파일 한도를 초과한다. anyOf 축소로도 미해결 → **ProjectGraph 생성에는 structured outputs를 쓰지 않는다.**
- 대신 **`ASSEMBLER_SYSTEM`의 "Return ONLY valid JSON … matching this shape" 지시 + 응답 추출(`extractJsonObject`: 펜스·프로즈 방어) + 정규화(ASS-019)** 로 유효 JSON을 확보한다. 정규화가 원래 "unknown→유효 ProjectGraph"의 진짜 보증이므로 안전.
- 프리필(마지막 assistant 턴) **금지** — 400(현 모델). messages 마지막은 항상 user.
- structured outputs(`outputSchema`) 능력은 `anthropic.ts`에 유지 — **작은 스키마**(예: ASS-067 라우터 턴 `{kind: clarify|generate}`)에는 문법 한도에 안 걸려 사용 가능.
- `max_tokens`는 큰 그래프 대비 넉넉히(비스트리밍 16000; 큰 그래프가 truncation 나면 ASS-068 스트리밍).
...
4. **eval 하네스 (ASS-062, 구현됨)** — `scripts/eval-generate.ts`(+`scripts/lib/eval-metrics.ts`)가 골든셋 대비 생성 품질 점수(coverage + normalizeHealth)·하드 무결성으로 프롬프트 변경 회귀를 측정. 계약·공식·타깃 단일 출처는 `docs/specs/prompt-department.md` §Evaluation Contract. 골든 self-score=1.0은 `scripts/verify-goldenset.ts`가 강제. 리포트: `docs/specs/diagnosis/eval-generate/<ISO>.json`.
